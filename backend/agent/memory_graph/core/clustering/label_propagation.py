...
对 Neo4j 中某用户的 Entity 节点做社区聚类。加权投票权重 = 语义相似度 * 0.6 + 关系连接 * 0.4。
迭代收敛后做社区合并（平均向量余弦 > 阈值），再用 LLM 生成社区名/摘要。
"""
import uuid
...
def _weighted_vote(neighbors: list[dict], self_emb: list[float] | None, labels: dict[str, str]) -> str | None:
    ...
    for nb in neighbors:
        ...
        weight = _SEM_WEIGHT * sem + _REL_WEIGHT * 1.0
...
class LabelPropagationEngine:
    ...
    async def _merge_communities(self, user_id: str, community_ids: list[str]) -> list[str]:
