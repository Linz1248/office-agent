...
from memory_graph.core.extraction.models import DedupDecision
from memory_graph.core.graph_models import EntityNode
...
async def _judge_same(client, a: EntityNode, b: EntityNode, ctx: dict) -> DedupDecision:
...
async def dedup_within_batch(client, entities: list[EntityNode]) -> tuple[list[EntityNode], dict[str, str]]:
...
async def merge_with_graph(
    client, repo: MemoryGraphRepository, user_id: str, entities: list[EntityNode]
