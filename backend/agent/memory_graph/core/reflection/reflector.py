...
from memory_graph.core.extraction.embedder import embed_texts
...
class ReflectionEngine:
    """反思引擎。chat_client 必需；embed_client 可选。"""

    def __init__(self, chat_client=None, embed_client=None):
    ...
    async def run(self, user_id: str) -> dict:
        ...
        entities = await self.repo.reflection_top_entities(user_id, settings.reflection_top_k)
        if len(entities) < settings.reflection_min_entities:
            logger.info("跳过反思（实体太少 %d<%d）: user=%s",
        ...
        for it in raw_insights:
            ...
            entity_ids = [
                ...
                for n in based_on if str(n).strip() in name_to_id
            ]
            try:
                await self.repo.upsert_insight(
                    user_id=user_id, theme=theme, content=content, embedding=embedding,
                    importance=_clamp(it.get("importance"), 0.6),
                    confidence=_clamp(it.get("confidence"), 0.7),
                    source_count=len(entity_ids), entity_ids=entity_ids,
                )
                saved += 1
            except Exception as e:
    ...
    async def _build_memory_block(self, user_id: str, entities: list[dict]) -> tuple[str, dict[str, str]]:
        lines: list[str] = []
